...
`playwright-bdd` (Gherkin → generated Playwright tests) · Winston (logging) ·
`dotenv`/`yaml`/`csv-parse`/`exceljs` (test data) · Allure (reporting) · one browser
project (`chromium`).

**What you end up with:** two independently runnable test layers, both written
BDD-style — UI (Page Object Model, against a demo e-commerce site) and API
(service-based, against a demo events API) — sharing one test-data system
(JSON/YAML/CSV/Excel, interchangeable), one secrets-resolution path, one logger, one
BDD generation pipeline (`features/**/*.feature` + `src/bdd/steps/**/*.steps.ts` →
native Playwright tests), and one CI workflow.
...
- [Phase 3b — BDD wiring (Gherkin + playwright-bdd)](#phase-3b--bdd-wiring-gherkin--playwright-bdd)
...
npm install --save-dev allure-commandline allure-playwright eslint @eslint/js \
  @babel/core @babel/eslint-parser @babel/preset-typescript
...
configured — this framework deliberately doesn't run cross-browser. `eslint-plugin-playwright`
is deliberately *not* installed: this framework's test bodies are `Given`/`When`/`Then` step
definitions (Phase 3b), not hand-written `test(...)`/`describe(...)` spec files, and the
plugin's rule set targets spec-file shapes it doesn't recognize in step definitions.
`playwright-bdd` is what turns Gherkin `.feature` files into native Playwright tests at
run time — see Phase 3b for how it's wired into `playwright.config.ts` and why the two
custom `test` fixtures (UI, API) both have to extend *its* `test`, not `@playwright/test`'s
directly.
...
```json
{
  ...
  "scripts": {
    "bddgen": "bddgen",
    "pretest": "rm -rf allure-results && npm run bddgen",
    "test": "playwright test",
    "presmoke": "rm -rf allure-results && npm run bddgen",
    "smoke": "playwright test --grep @smoke",
    "preregression": "rm -rf allure-results && npm run bddgen",
    "regression": "playwright test --grep @regression",
    "preapi": "rm -rf allure-results && npm run bddgen",
    "api": "playwright test .features-gen/api",
    "preui": "rm -rf allure-results && npm run bddgen",
    "ui": "playwright test .features-gen/ui",
...
report mixing tests that didn't actually run this time with ones that did. Every
`pre<script>` also runs `npm run bddgen` (Phase 3b) — this framework's actual test files
are generated from `features/**/*.feature` + `src/bdd/steps/**/*.steps.ts` at run time, so
regenerating them has to happen before Playwright even starts, not as a separate manual
step a contributor could forget. `npm test` itself doesn't need its own explicit filter
because Playwright's default is "run everything under `testDir`" — the plain
`playwright test` in the `test` script.
...
```json
{
  ...
  "include": ["src/**/*.ts", "config/**/*.ts", "playwright.config.ts"],
  "exclude": ["node_modules", "allure-report", "allure-results", "test-results", "playwright-report", ".features-gen"]
}
```
...
`include` is scoped deliberately to `src/`, `config/`, and the config file itself — not
the whole repo — so generated output (`playwright-report/`, `allure-results/`,
`test-results/`, `.features-gen/`, and anything under `node_modules`) never gets
type-checked; those directories don't contain source TypeScript and including them
would slow every `tsc` invocation for no benefit. `src/` covers the step definitions
under `src/bdd/steps/` (Phase 3b) too, so there's no separate `tests/**/*.ts` entry —
this framework has no hand-written spec files, only Gherkin `.feature` files (not
TypeScript, and not type-checked) and the generated `.spec.js` under `.features-gen/`
(plain JS, explicitly excluded — it's regenerated on every run, never hand-edited).
`npm run typecheck` is added immediately, before there's any real application code,
specifically so CI (Phase 8) enforces it starting with the very first commit rather
than retrofitting a type-check gate onto a codebase that's already accumulated
violations.
...
imports, or accidental `var`. ESLint fills that gap. The two-block structure (plain JS →
all `.ts`) is a layered configuration where later blocks in the flat-config array add to,
rather than replace, earlier ones for files they also match:
...
1. The first `.{js,mjs,cjs}` block applies `eslint:recommended` — this file itself
   ...
   (`.features-gen/**`, the generated `.spec.js` output, is excluded up front — it's
   regenerated on every run, never hand-edited, so linting it would be pure noise).
2. The `**/*.ts` block is where everything in `src/` and `config/` gets linted, but
   deliberately *not* via `eslint:recommended` — that ruleset assumes plain-JS shapes
   (every function has a body, every property has a value) and throws parser-level
   errors on TypeScript-only syntax with no body, like an abstract method signature in
   `BaseDataProvider` (Phase 3: `protected abstract parse<T>(filePath: string):
   Promise<T>;`) or an interface member. So instead of pulling in `typescript-eslint`'s
   type-aware parser (blocked here because this project pins TypeScript 7, ahead of what
   `typescript-eslint` currently supports), `.ts` files are parsed with Babel's
   TypeScript preset — good enough to build an AST for style rules, with no actual type
   information. That's why the ruleset here is small and hand-picked rather than a
   wholesale `recommended` set: every rule had to be manually checked for false
   positives against a parser that can't see types.

There's deliberately no third, Playwright-specific block layered on top of `src/bdd/steps/`
the way an earlier version of this config layered `eslint-plugin-playwright` onto
`tests/**/*.ts`: that plugin's rules (`expect-expect`, `no-standalone-expect`, etc.)
assume a spec-file shape — `expect(...)` calls sitting directly inside a `test(...)`
callback — and step definitions don't have that shape; a `Then(...)` step's `expect(...)`
call sits inside a step function, not a `test()` body, so `no-standalone-expect` flags
every single assertion in every step file as a false positive. Assertions in this
framework are centralized in `src/validators/*.ts` (Phase 3) anyway and called as e.g.
`LoginValidator.expectLoginSuccess(...)` from a `Then` step — plain TS/JS lint rules
(`no-var`, `eqeqeq`, etc.) are what actually apply to that code, and they already come
from the `**/*.ts` block above.
...
# BDD (playwright-bdd generated specs — regenerated by `npm run bddgen`, Phase 3b)
.features-gen/
...
from `use.screenshot`/`trace`/`video` in `playwright.config.ts`), `playwright-bdd`
(`.features-gen/` — the generated `.spec.js` files `npm run bddgen`, Phase 3b, writes;
they're build output derived entirely from `features/` + `src/bdd/steps/`, never
hand-edited, so there's nothing to review or commit), Allure (`allure-results/`,
`allure-report/` — the two directories `npm run allure-report`, Phase 7, generates into
and reads from), the TypeScript compiler's incremental-build cache (`*.tsbuildinfo` —
not actually produced here since `noEmit: true`, but harmless to ignore preemptively),
and Winston's own output (`logs/`, from `Logger.ts`, Phase 2). Grouping by "what tool
wrote this" rather than alphabetizing makes it obvious, when a new tool is added later,
exactly where its ignore rule belongs.
...
`defineBddConfig()`'s `featuresRoot` option requires the directory to already exist on
disk (it fails fast with `Config option 'featuresRoot' should be a directory'` otherwise)
— so create the two BDD folders now, empty, before writing this file. Phase 3b fills
them in with real `.feature`/`.steps.ts` files; nothing here depends on their contents
yet, only on the directories existing:

```bash
mkdir -p features src/bdd/steps
```
...
// test run (see Phase 0's `pre*` hooks). Each `.feature` file's steps are
...
// Full rationale for every option here is in Phase 3b — this file only needs
// to exist this early because `testDir` below has to point somewhere real.
...
- `testDir` (the `defineBddConfig()` return value, resolving to `.features-gen`) — this
  is what physically separates "generated test specs" from everything else in the repo.
  Only the `.spec.js` files `bddgen` writes under `.features-gen/` are ever picked up as
  test files — page objects, services, providers, step definitions, and every other
  `.ts` file elsewhere in `src/` are plain modules that the *generated* specs import,
  never executed by Playwright's own discovery directly. This is also why `npm run ui`
  and `npm run api` (Phase 7) can safely pass `.features-gen/ui` / `.features-gen/api` as
  extra path filters — they're narrowing an already-scoped `testDir`, not redefining it.
  There's no explicit `testMatch` override: Playwright's default pattern already matches
  the `.spec.js` extension `bddgen` writes.
- `timeout: ENV.DEFAULT_TIMEOUT` / `expect.timeout: ENV.EXPECT_TIMEOUT` — two *different*
  ...
  file alone.
- `fullyParallel: true` — tests within the *same generated spec file*, not just across
  files, run concurrently across workers. This is what makes the strict "assertions live
  in validators, page objects hold no shared mutable state" discipline (Phase 4) actually
  matter: if two scenarios generated from `login.feature` shared a page object instance
  or a module-level variable, running them in parallel would produce flaky cross-test
  interference. Each test instead gets its own fresh `page` (and therefore fresh
  page-object instances, via the `testFixture`) from Playwright itself — which is also
  why a BDD step file's own module-scoped state (Phase 3b) is safe only *within* one
  scenario's `Given`→`When`→`Then` chain, never shared across scenarios.
...
why folder creation belongs here and not, say, in the lifecycle-logging fixture
(Phase 4): creating
...
**Checkpoint:** `npx playwright test` runs `globalSetup` (the banner prints, `logs/` gets
created) and `defineBddConfig()` loads without the `featuresRoot` error, then reports
`Error: No tests found` and exits 1 — expected at this point, since `features/` is still
empty; Phase 3b's `bddgen` output is what finally gives it something to run.
...
**Why this needs to exist at all:** `src/bdd/steps/api/event.steps.ts` (Phase 5) creates
an event with `eventDate: getFutureDateIso(2, {...})` — "2 days from whenever this
scenario happens to run." A hardcoded date string in the dataset (`event.createEvent.eventDate:
"2027-06-15T..."`, as it appears literally in `apiData.json`) works as a fallback
default value inside the JSON, but the step deliberately *overrides* it at runtime with
...
milliseconds: 0 }` in `event.steps.ts` — pinning the event to exactly 9:00:00.000 AM UTC
...
API engine, step definitions), so re-exporting them from one `src/utils` barrel is
convenient.
...
(and possibly typed slightly differently in different step files, which would make some
...
constants are consumed *by validators*, not by step definitions directly, keeping "what
the app should say" and "where that gets asserted" both centralized rather than
scattered.
...
**Why these exist as named constants at all, when a `.feature` file could just write the
literal tag `@smoke` above a `Scenario:`:** Playwright's `--grep`/`--grep-invert` (and
this framework's `npm run smoke`/`npm run regression` scripts, Phase 7) match against a
generated test's tags — and `playwright-bdd` (Phase 3b) turns a Gherkin `@smoke` tag
directly into that Playwright tag, no extra wiring needed. There's no dedicated
Playwright "tag" API these constants plug into on the Gherkin side either; `TestTags.SMOKE`
is purely a typed, autocomplete-friendly, typo-proof reference to the string `"@smoke"`
for anywhere in the codebase that needs to *refer to* a tag programmatically (for
instance, a future helper that filters or groups tests by tag). Directly in a `.feature`
file, the tag is still written as the plain literal (`@smoke` above `Scenario: Valid user
should login successfully` in `login.feature`, Phase 4) because that's what Gherkin's tag
syntax — and Playwright's grep matching underneath it — actually scans; the plain literal
is what's used by convention here since a Gherkin tag is inherently a one-off token, not
shared code that benefits from a named reference. `Object.freeze` again guards against a
...
- `this.cache.set(cacheKey, data)` — caches the *already-secret-resolved* data, not the
  ...
  many different step files call `TestData.load("uiData")`, which matters because a
  tag-scoped `BeforeAll` hook in *each* step file independently calls
  `TestData.load(...)` (Phase 3b/4) and there's no reason to repeat file I/O and
  placeholder resolution on every one of those calls.
...
exchange for `TestData.load()` and every step file's `BeforeAll` hook having exactly
one call shape (`await TestData.load(...)`) that works identically no matter which
...
the *only* class in the whole codebase that a step definition ever imports to get test
data — no step file ever imports `DataProviderFactory` or a concrete provider directly.
That single choke point is what makes the entire four-format architecture invisible to
test authors: a step file just writes `await TestData.load<UiData>("uiData")` and has
no idea, and no need to know, whether that's reading JSON, YAML, CSV, or Excel
underneath.
...
A barrel with exactly one re-export might look pointless, but it's what makes
`import { TestData } from "<relative path to>/data"` (as every step file does, Phase
4/5 — the exact relative path just depends how deep under `src/bdd/steps/` that
particular step file lives) work without callers needing to know or care that
`TestData` specifically lives in `src/data/TestData.ts` — if the internal file layout of
this module ever changed, every step file's import statement would keep working
unchanged.
...
the codebase that consumes the loaded data (`login.steps.ts`, `checkout.steps.ts`,
Phase 4) gets full autocomplete and compile-time checking against this declared shape.
If a step definition tries to access `uiData.login.nonExistentUser`, that's a `tsc`
error (Phase 0's `typecheck` script) rather than a runtime `undefined` discovered only
when the scenario actually runs. The three login variants
(`validUser`/`invalidPassword`/`invalidEmail`) directly mirror the three login scenarios
in `login.feature` — this type and that feature/steps pair were designed together, one
describing the shape the other consumes.
...
dataset a step file loads — and re-reading and re-parsing the secrets file on every
single call would be wasted, repeated file I/O for data that never changes mid-run. Reading it
...
## Phase 3b — BDD wiring (Gherkin + playwright-bdd)

Both the UI and API layers below are written BDD-style: a scenario is a Gherkin
`.feature` file, and its behavior is implemented as `Given`/`When`/`Then` step
definitions. Neither layer contains a single hand-written `test(...)` call anywhere —
`playwright-bdd` reads the `.feature` files plus the step definitions and *generates* a
real Playwright spec file per scenario at run time. This phase covers the shared
machinery both layers build on; Phase 4 and Phase 5 each show their own feature files
and step definitions on top of it.

### The three-file relationship: `.feature` → `.steps.ts` → generated `.spec.js`

```text
features/ui/authentication/login.feature   (Gherkin: what the scenario does)
        ↓ matched by text against
src/bdd/steps/ui/authentication/login.steps.ts   (Given/When/Then implementations)
        ↓ bddgen generates
.features-gen/ui/authentication/login.feature.spec.js   (native Playwright test, gitignored)
```

Directory layout mirrors the old UI/API split one-to-one: `features/ui/<feature>/*.feature`
+ `src/bdd/steps/ui/<feature>/*.steps.ts` for UI (per-feature subfolders), flat
`features/api/*.feature` + `src/bdd/steps/api/*.steps.ts` for API — the same split
`tests/authentication`/`tests/checkout` (nested) vs. flat `tests/api` would have had in
a non-BDD version of this framework. `src/bdd/steps/` mirrors `features/` path-for-path
(swap `.feature` → `.steps.ts`) so anyone looking at a scenario knows exactly where its
implementation lives without searching.

### `playwright.config.ts`'s `defineBddConfig()` (Phase 1), explained
...
- `features` — every `.feature` file matching this glob becomes one or more generated
  tests; `bddgen` (`npm run bddgen`, wired into every `pre*` npm hook from Phase 0) reads
  them fresh on every run, so a `.feature` file is a live source of truth, not a
  one-time scaffold.
- `steps` — step-definition files `bddgen` scans to match `Given`/`When`/`Then` text
  against each scenario's steps. The two fixture files (`src/fixtures/testFixture.ts`,
  `src/api/fixtures/apiTest.ts`, Phase 4/5) are listed here *directly*, not just
  transitively imported by the step files — `bddgen` needs to find where `test` is
  actually exported from (via `base.extend(...)`, see below) to know which fixtures a
  generated scenario should import, and it only looks for that inside files matched by
  this `steps` glob.
- `featuresRoot: "features"` — strips the `features/` prefix from generated paths, so
  `features/ui/authentication/login.feature` generates to
  `.features-gen/ui/authentication/login.feature.spec.js` rather than nesting an extra
  `features/` level inside `.features-gen/`. It also has to point at a directory that
  already exists on disk (Phase 1's `mkdir -p features src/bdd/steps`) — `bddgen` fails
  immediately, before reading anything else, if it doesn't.

### The `test` instance step definitions build on must come from `playwright-bdd`

```ts
// src/fixtures/testFixture.ts / src/api/fixtures/apiTest.ts
import { test as base } from "playwright-bdd"; // not "@playwright/test"
```

`createBdd(test)` (used in every step-definition file, Phase 4/5) requires the `test`
instance it's given to descend from `playwright-bdd`'s own `test` — which is itself
`@playwright/test`'s `test` extended with `playwright-bdd`'s internal fixtures
(`Given`/`When`/`Then` themselves, `$bddContext`, `$tags`, and others prefixed `$` to
avoid colliding with this framework's own fixture names). Extending `@playwright/test`'s
bare `test` directly, the way a non-BDD version of this framework would, produces a
`test` instance `createBdd()` rejects outright (`createBdd() should use 'test' extended
from "playwright-bdd"`) — so both `src/fixtures/testFixture.ts` (Phase 4) and
`src/api/fixtures/apiTest.ts` (Phase 5) import `test as base` from `"playwright-bdd"`,
not `"@playwright/test"`, as the very first line of their own `base.extend(...)` chain.
`expect` is unaffected by this and still comes from `"@playwright/test"` directly — only
the `test` type itself needs to originate from `playwright-bdd`.

### Why per-test logging can't be a `test.beforeEach()`/`test.afterEach()` call here

A non-BDD version of this framework would register its logging hooks with
`test.beforeEach(...)`/`test.afterEach(...)`, called at the module scope of the fixture
file, immediately after `test` is created. That exact pattern breaks under BDD: `bddgen`
loads every file matched by the `steps` config — step-definition files, and (per above)
the fixture files they import `test` from — to *discover* what steps and fixtures exist,
before it has generated or run a single actual test. At that point in `bddgen`'s own
execution there is no active Playwright suite on the stack yet; calling
`test.beforeEach()` while loading a file in that context throws immediately:

```text
Error: Playwright Test did not expect test.beforeEach() to be called here.
```

The fix (Phase 4's `lifecycleLoggingFixture`) is to express the same "run before/after
every test" behavior as a plain Playwright **fixture** instead — specifically an
*auto-fixture* (`{ auto: true }`, so every test picks it up without needing to
destructure it) that does its "before" work, calls `await use()`, then does its "after"
work. A `.extend({...})` call, unlike `test.beforeEach()`, is just building a fixture
definition object — it doesn't touch the suite stack at module-load time, so it's safe
in both `bddgen`'s discovery pass and a genuine test run.

### Worker-scoped `BeforeAll`/`AfterAll` hooks must be tag-scoped

`createBdd(test)` also returns `BeforeAll`/`AfterAll` (and scenario-scoped
`Before`/`After`) — the BDD equivalents of Playwright's own worker/test hooks, used in
Phase 4/5 to load each layer's dataset once via `TestData.load()`. Unlike a hook
registered inside one specific `.feature` file's own generated test, a `BeforeAll`/`AfterAll`
hook is a **global** lifecycle hook by default: `bddgen` includes it when generating
*every* feature file's spec, regardless of whether that feature's scenarios use the
step file the hook was declared in. With two unrelated custom `test` instances in this
framework (the UI layer's and the API layer's), an unscoped `BeforeAll` in, say,
`login.steps.ts` gets pulled into the API layer's generated spec too — and since the UI
and API `test` instances don't extend each other, `bddgen` can't resolve which one a
scenario should actually use, and fails with:

```text
Error: Can't guess test instance for: features/api/event.feature. Found 2 test
instances, but they should extending each other. Please check BDD configuration "steps"
or set "importTestFrom" manually.
```

The fix is to scope every `BeforeAll`/`AfterAll` to the layer's own tag — `BeforeAll({
tags: "@ui" }, async () => {...})` in UI step files, `BeforeAll({ tags: "@api" }, async
() => {...})` in API ones — which is also why every `Feature:` in this framework carries
`@ui` or `@api` as its first tag (Phase 4/5): it's not just documentation, it's what
scopes these hooks correctly.

### Scenario Outline for data-driven variations

Gherkin's `Scenario Outline` + `Examples` (used for the two negative-login cases in
`login.feature`, Phase 4) generates one test per `Examples` row instead of requiring one
hand-written `Scenario` per data variation. Naming the outline itself with the column
placeholder (`Scenario Outline: Invalid <credentialKey> should display a login error`)
gives each generated test a readable title built from that row's actual value, instead
of `bddgen`'s default `Example #1`/`Example #2` — `playwright-bdd` substitutes any
`<columnName>` it finds in the scenario's own name from the matching `Examples` column.

**Checkpoint:** `npm run bddgen` exits `0` with no output — `features/` and
`src/bdd/steps/` both still exist but are empty at this point in the build, so there's
nothing to generate yet and nothing to fail on either.
...
validators → feature files + step definitions. Assumes Phase 3b's `defineBddConfig()`
wiring and folder layout are already in place.
...
top of it, and every scenario that exercises that page keeps working completely
unchanged.
...
actually needs across this whole framework's scenarios — nothing here has ever needed a
...
framework's own scenarios (the checkout flow here has no checkbox interactions) — it exists
...
- `protected async navigate(url)` — `protected`, not `public`, deliberately: a step
  definition should never be able to call `basePage.navigate("/some/arbitrary/url")` directly on
...
- `reload()`/`currentUrl()` — thin, direct pass-throughs to the underlying `Page`.
  These exist purely so a step definition never needs to reach for the raw `page`
  fixture at all — everything a step does goes through a page-object method, keeping
  the raw Playwright `Page` object entirely an implementation detail page objects hide.
- `verifyUrl`/`verifyTitle` — these *do* wrap `expect(...)`, which might look like it
  ...
  assertions about test outcomes. In practice, this framework's actual step definitions
  use the dedicated validators (`LoginValidator.expectLoginSuccess`, which checks `currentUrl()`
...
- **`private readonly` fields:** nothing about a page object's internal component
  ...
  outside the class (`private`) — a step definition interacts with `LoginPage` purely
  through its public methods (`login`, `getErrorMessage`, `navigate`), never by
  reaching in to grab `loginPage.loginButton` directly. This is the actual mechanism
  that keeps "how the login page's DOM is structured" fully encapsulated.
- **`navigate()` overriding the inherited `protected navigate(url)`:** this public
  method is what a `Given`/`When` step actually calls (`await loginPage.navigate()`),
  taking no arguments — the URL itself (`AppRoutes.LOGIN`) is baked in here, not
  something a caller supplies, because *this* page object only ever represents *the*
  login page.
- **Why `login()` wraps each step in `AllureHelper.step(...)` instead of just three
  ...
  pass/fail-attributable steps nested under the generated test, rather than one opaque
  `login()` call — when a test fails partway through, the report immediately shows
  which specific sub-step failed, without needing to read the raw log to figure that
  out.
- **`login(user: User)` taking the shared `User` domain type, not two separate
  `email`/`password` string parameters:** this is what lets a step definition write
  `await loginPage.login(user)` where `user` came straight out of
  `structuredClone(uiData.login.validUser)`
...
- **Why `waitForLoad()` is overridden here specifically to `page.waitForURL(/\/dashboard\/dash/)`
  ...
  is exactly why `login.feature`'s positive scenario's `Then` step calls `waitForLoad()`
  right after the `When` step's `login()` before checking the final URL.
...
There's no pressure to pre-build methods for things no scenario actually exercises yet
(viewing cart contents, removing an item) — a page object grows exactly as far as the
scenarios that need it, which keeps every file here honestly reflecting what's actually
...
### Validators — assertions live here, not in page objects or step definitions
...
every `Then` step explicit and readable: `LoginValidator.expectLoginSuccess(await
...
instead of a `Then` step calling `expect()` directly on the boolean:** this might look
like pure ceremony around a trivial check, but it's consistency, not laziness — every
business-outcome assertion in this framework goes through a named validator method, no
exceptions, so a reviewer scanning a `Then` step in `checkout.steps.ts` (Phase 4) always
sees a readable, named call (`CheckoutValidator.expectOrderInHistory(...)`) instead of a
bare boolean check whose meaning has to be inferred from context. A step that
occasionally called bare `expect()` directly for "simple enough" checks would silently
erode that convention.
...
**Why this is one auto-fixture wrapping the whole test, rather than the separate
`beforeEach`/`afterEach` pair a non-BDD version of this framework would use:** Phase 3b
already covers *why* it can't be `test.beforeEach()`/`test.afterEach()` — this is that
constraint's concrete shape. A Playwright fixture function is a single generator-like
async function: everything before `await use()` runs as the "before" half, `use()` hands
control to the test body (and everything downstream that depends on this fixture, since
`{ auto: true }` makes it apply to every test automatically without being destructured),
and everything after `await use()` runs as the "after" half once the test — and
everything nested inside it — has finished. One function replaces what would otherwise
be two separately-registered hooks, because a fixture's shape *is* a before/after pair
by construction.

**Why `Logger.setContext(...)` is set as literally the first line before `await use()`:**
...
**Why `Logger.setContext("")` runs last, right before the fixture function returns:**
since `currentContext` is one shared, mutable, module-level value (Phase 2), it *must*
be explicitly cleared after every test — otherwise the *next* test to run in this same
worker process would inherit the previous test's now-stale context prefix on its own
early log lines, right up until this same fixture overwrites it again for that next
test. Clearing it here closes that window entirely: there's never a moment between two
tests in the same worker where a log line could be mislabeled. This is also, concretely,
why the "after" logging has to live in the same fixture function as the "before"
logging rather than a separate one: `await use()` is the one point in this file that's
guaranteed to run after the entire test (and anything nested under it) has completed,
in the same scope where the "before" half already ran.
...
hardcoded `"passed"` is what keeps this fixture from spamming `logs/error.log` with
...
**Why the explicit `{ frameworkLifecycleLogging: [...] }` type annotation on
`lifecycleLoggingFixture`, rather than letting TypeScript infer it:** without it, TS
widens the two-element `[fn, { auto: true }]` tuple TypeScript's fixture API expects
into a plain, mutable array — `((fixtures, use, testInfo) => ... ) | { auto: boolean }`
— which no longer satisfies `base.extend()`'s parameter type once it's spread into
`testFixture.ts`/`apiTest.ts`'s fixture object below. The explicit tuple type keeps the
literal shape TypeScript needs to see.
...
**Why `base.extend<FrameworkFixtures>({...})` is the mechanism, rather than each step
definition constructing its own page objects by hand:** Playwright's fixture system
...
fresh `LoginPage` is created from that test's own `page` *only if a step actually
destructures `loginPage` as a parameter*, and Playwright handles creating, providing,
and disposing of it with no manual setup/teardown code in any step file. This is what lets
`login.steps.ts` write `async ({ loginPage }) => {...}` in each step and get a
fully-wired page object with zero boilerplate, while `checkout.steps.ts` can
destructure any of *six* fixtures across its different steps (`loginPage, homePage,
cartPage, checkoutPage, orderConfirmationPage, ordersPage`), each independently created
from the same underlying `page`, letting one scenario's steps flow naturally across the
objects representing each page it visits.

**Why every page-object fixture function follows the identical `async ({ page }, use)
=> { await use(new X(page)); }` shape:** this uniformity isn't accidental — it's what
makes adding the *next* page object purely mechanical: extend the `FrameworkFixtures`
type with one new field, add one fixture function following the exact same pattern,
and it's immediately available to every step file that imports `test` from this file.
There's no special-casing needed regardless of how many page objects the fixture
eventually holds.

**Why `...lifecycleLoggingFixture` is spread into the fixtures object here, rather than
called as a function the way a non-BDD version's `registerTestHooks(test)` would be:**
Phase 3b covers why the logging behavior itself has to be a fixture, not a
`test.beforeEach()`/`test.afterEach()` call; this is the mechanical consequence — a
fixture is *declared*, as an entry in the object passed to `.extend()`, not invoked
separately afterward the way a hook-registering function call would be. Spreading it in
here guarantees that *every* step file that imports `test` from this file automatically
gets the logging fixture (since it's `{ auto: true }`, Phase 3b) with zero chance of a
step author forgetting to wire it up — it's an inherent property of importing this
fixture, not an opt-in step.

**Why `expect` is re-exported from here instead of step files importing it from
`@playwright/test` directly:** this is a small but important discipline-enforcing
choice — if a step file imports `test` from `../../../../fixtures/testFixture` but
`expect` from `@playwright/test` directly, it still works today (they're the same
underlying `expect`), but it invites a step file to also import `test` from the wrong
place later, since there'd be no established habit of "everything test-related comes
from the fixture file." Re-exporting `expect` here means a step file's import line is a
single, consistent source for both, with the fixture file as the one deliberate seam
between this framework's step definitions and raw Playwright.

### Feature files + step definitions — never a hand-written `test(...)` anywhere
...
**Why `Background: Given I am on the login page` instead of repeating that step inside
each `Scenario`:** all three scenarios need the same starting point, and Gherkin's
`Background` runs before every scenario in the feature file automatically —
`playwright-bdd` (Phase 3b) turns it into `test.beforeEach(...)` in the generated spec.
This is a purely Gherkin-level convenience with no equivalent decision to make on the
non-BDD side, since a plain spec file would have had each test call
`loginPage.navigate()` individually instead.

**Why the two negative scenarios are one `Scenario Outline` with an `Examples` table
instead of two separately-written `Scenario`s:** Phase 3b covers the mechanism; the
payoff here is concrete — `invalidPassword` and `invalidEmail` exercise the exact same
`When`/`Then` steps with one different input, so writing them as two near-identical
`Scenario`s would mean any future change to those two steps has to be made twice,
correctly, in sync. Naming the outline `Invalid <credentialKey> should display a login
error` means the two generated tests are titled `Invalid invalidPassword should
display a login error` / `Invalid invalidEmail should display a login error` — as
readable in a report as two hand-written scenario titles would have been.

**Why `resolveLoginUser` looks the credential key up in a `Record<string, User>` built
from `uiData.login.*` instead of the `.feature` file just writing the email/password
literally in the `When` step's text:** this is the direct, load-bearing consequence of
the secrets rule (Phase 3a) once tests live in Gherkin — a `.feature` file is a
plain-text source file like any other, checked into version control, so a real
credential (or the placeholder-resolved value it becomes at runtime) can never appear
in its step text. Passing a *name* (`"validUser"`) through the step and resolving the
actual `User` object inside the step definition, against data already loaded via
`TestData.load()` (Phase 3), keeps the `.feature` file itself free of anything
resolvable back to a secret — it only ever says which named credential set to use, never
what's in it.

**Why `BeforeAll({ tags: "@ui" }, ...)` rather than an untagged `BeforeAll(...)`:** Phase
3b's tag-scoping rule made concrete — this hook is bound to the UI layer's `test`
instance (via `createBdd(test)` above), and without the `{ tags: "@ui" }` scope,
`bddgen` would try to pull it into the API layer's generated spec too and fail to
resolve which `test` instance `event.feature` (Phase 5) should use.

`features/ui/checkout/checkout.feature` — a full multi-page-object journey through
...
`src/bdd/steps/ui/checkout/checkout.steps.ts`:
...
**Why this is one scenario with seven steps rather than split into several smaller
scenarios (one for search, one for add-to-cart, one for checkout, etc.):** this is a
genuine, end-to-end user journey — log in, search, add to cart, check out, confirm,
verify in history — where each step's precondition is the *previous* step's outcome;
there's no independently-meaningful "checkout succeeded" state without first having
logged in, searched, and added an item. Splitting it into isolated scenarios would
require each one to redundantly re-run every earlier step just to reach its own
starting point, multiplying total execution time for no added coverage — this is
precisely why it's tagged `@e2e` (Phase 2's `TestTags.E2E`) as well as `@smoke`, marking
it explicitly as an end-to-end journey rather than a narrow unit-style UI check.

**Why `productName` is a module-scoped `let`, searched for in one `Given` step and read
again three steps later in the final `Then`, rather than being re-derived or passed
explicitly between steps:** unlike the API layer's `ApiScenarioContext` (Phase 5), BDD
step definitions have no framework-provided per-scenario storage — Phase 3b covers why
this is safe regardless: `fullyParallel: true` (Phase 1) runs different *scenarios*
concurrently across workers, never two steps of the *same* scenario concurrently within
one worker, so a module-scoped variable set in one step and read in a later step of that
same scenario never collides with another scenario's run, even a concurrently-running
one in a different worker (each worker loads its own copy of this module).

**Why the step-file destructuring across this file's six `Given`/`When`/`Then` functions
covers all six page-object fixtures between them, more than any single step in the
file needs on its own:** this is the practical payoff of the fixture design
(`testFixture.ts`, above) — each step just lists the fixtures *it* needs
(`{ loginPage }`, `{ homePage, cartPage }`, `{ checkoutPage }`, ...) and gets
fully-wired, ready-to-use instances, with no manual construction, no passing `page`
around by hand between helper functions, and no risk of accidentally reusing a stale
page-object instance across steps — Playwright still creates all six from the one
underlying `page` for the one running scenario, exactly as it would for a single test
function destructuring all six at once.

**Why `({ productName } = structuredClone(uiData.checkout))` destructures immediately
after cloning, rather than cloning `productName` on its own:** cloning the parent
`checkout` object once and destructuring the field out of the clone is simpler and
marginally cheaper than structuring the clone around one field in isolation — and since
`payment` (a `PaymentDetails` object, read in a later step) and `productName` come from
the same `uiData.checkout` object, treating it as one logical unit to clone matches how
the dataset is actually shaped.

**Checkpoint:** `npm run bddgen && npx playwright test .features-gen/ui` passes against
a real app — three generated tests: the smoke-tagged login scenario, the two examples
generated from the negative-login outline, and the checkout journey.
...
fixture → feature file + step definitions. Assumes Phase 3b's `defineBddConfig()`
wiring and folder layout are already in place.
...
`response.body` to their own callers, so step definitions never see the envelope at all
— they work with plain `LoginResponse`/`EventResponse` objects — while `ApiEngine` internally
...
catch block anywhere in a step definition or elsewhere that needs to inspect a failed
API call (say, a scenario that deliberately expects a `ValidationException` and wants to check its
...
correctly to "only one token *per test*," which matches how this framework's scenarios
actually work: `event.feature`'s scenario logs in as exactly one user for the duration
of that one generated test. A scenario genuinely needing two simultaneous authenticated
identities in one test would need two separate `TokenManager`/`ApiEngine`/`ApiFacade`
sets, which the fixture as written doesn't currently provide — that's a real,
acknowledged scope limit of the current design, not something this class tries to solve
generically.
...
to the caller:** `event.steps.ts` (below) never manually manages the token itself
...
alternative (a step manually calling `tokenManager.setToken(loginResponse.token)` after
every login) would be one more easy-to-forget step repeated in every single API step
file that needs authentication.
...
**Why this class exists at all, when a step definition could just use a local `const`
variable for values like an event ID:** in a hand-written spec file, a single test
function *could* just write `const eventId = createEventResponse.data.id` and close
over that local variable for the rest of the test body — but `event.steps.ts` (below)
doesn't have that option: `Given`/`When`/`Then` are separate, independently-invoked
functions (Phase 3b), so an event ID computed inside one `When` step's function body is
already out of scope by the time the next `Then` step's function runs. A plain local
variable *cannot* cross that boundary; `api.setContextValue(...)`/`api.getContextValue(...)`
can, since the `ApiFacade` instance (and its context) is the one thing threaded through
every step of the scenario via the `api` fixture, created once per test and reused
across all of that test's steps. It also gives every stored value a debuggable name
(`"authToken"`, `"eventId"`) rather than relying on however local variables happen to
be named in a given step, which matters if this context were ever logged or inspected
for debugging.
...
service map and scenario context behind it — rather than a step definition directly
importing `AuthenticationService`/`EventService`/`ApiScenarioContext` itself:**
`event.steps.ts` (below) never imports any service class or `ApiScenarioContext`
directly — it only ever touches the single `api` fixture value, calling
`api.service("auth")`, `api.setContextValue(...)`, etc. This is deliberate
encapsulation: if this framework ever restructured how services are constructed or
wired (say, injecting a shared cache or a different retry policy per service), every
step file keeps working unchanged as long as `ApiFacade`'s own public surface
(`service`, `setContextValue`/`getContextValue`/etc.)
...
`event.feature`'s scenario never accidentally starts already-authenticated from some
*previous* test's login — every API test starts from a completely clean slate: no token, no
...
`src/api/fixtures/apiTest.ts` — reuses the same `lifecycleLoggingFixture` from the UI
layer, since logging isn't UI-specific:
...
This mirrors `src/fixtures/testFixture.ts` (Phase 4) exactly in the two BDD-specific
respects: `test as base` comes from `"playwright-bdd"`, not `"@playwright/test"` (Phase
3b — `event.steps.ts`'s `createBdd(test)`, below, requires it), and
`...lifecycleLoggingFixture` is spread in rather than called as a separate
`registerTestHooks(test)` function (Phase 4 — a fixture, not a `test.beforeEach()` call,
is what survives `bddgen`'s step-discovery pass).
...
`createApiFixture` returns both `api` and `requestContext`:** a step definition never
needs the raw `requestContext` directly — it only ever interacts through `api.service(...)`,
...
`src/constants`, and `src/data`. In practice, most consumers (the step definitions)
import more specifically from `src/api/fixtures/apiTest` for `test`/`expect`, since
that's the one thing every API step file actually needs; this top-level barrel matters
more for anything that needs the lower-level building blocks (a service, an exception
class) directly.
...
`src/bdd/steps/api/event.steps.ts` — never calls `request.newContext()` or a raw HTTP
verb directly; everything goes through `api.service("<name>")`:
...
**Why this step file asserts with raw `expect(...)` calls directly inside `Then` steps,
unlike the UI layer which routes every assertion through a validator class:** this is a
real, deliberate asymmetry between the two layers worth calling out rather than
glossing over. The UI layer's validators exist primarily to keep expected *application
copy* (exact error message text, success message wording) out of page objects and
centralized in one place (`Messages`, Phase 2) — UI assertions are fundamentally about
matching human-facing text that's prone to wording changes. API assertions here are
checking *structural* facts about a JSON response (`success === true`, `data.id > 0`,
...
same pattern if these checks grew more complex or needed reuse across multiple step
files, but for this one flow's needs, inline `expect()` calls inside each `Then` step
are simpler and equally clear.

**Why `buildEventPayload` recomputes `eventDate` via `getFutureDateIso` on every call,
rather than computing one date once and reusing it for both create and update:** each
call independently spreads `{...base, eventDate: getFutureDateIso(2, {...})}`, keeping
every other field from the dataset (`title`, `description`, `category`, etc.) exactly
as authored — this is the spread pattern for "take this test-data object, but override
just the one runtime-computed field," which keeps the vast majority of the payload
declaratively defined in the dataset file while only the one genuinely time-sensitive
field is computed at run time. Both the `When I create an event` and `When I update the
event` steps call it with identical arguments (`2` days out, pinned to `09:00:00.000`)
rather than different offsets, since there's no requirement here that create and update
use different dates — it's simply that each payload needs its *own* independently
future-valid date rather than literally sharing one JS object reference.

**Why the scenario flows strictly login → create → update → delete across eight
separate `Given`/`When`/`Then` steps, with every value threaded through
`api.setContextValue(...)`/`api.getContextValue(...)` rather than a shared local
variable:** this directly exercises the full lifecycle of a resource this scenario
itself creates, and critically, it also **cleans up after itself** — the final `When I
delete the event` step means this scenario doesn't leave orphaned event records behind
in the backend after every run, which matters for a scenario that might run repeatedly
(every CI run, every local run) and would otherwise accumulate junk data over time.
Unlike the equivalent single-test-function version of this scenario (a plain `const
eventId = createEventResponse.data.id` closing over the rest of the test body), threading
every value through the scenario context here isn't optional ceremony — Phase 3b covers
why: `Given`, `When`, and `Then` are separate, independently-invoked functions, so a
value computed inside one step's function body is out of scope by the time the next
step's function runs. `api.setContextValue("eventId", ...)` in the `When I create an
event` step and `api.getContextValue<number>("eventId")` in every step downstream is
the only way this data actually crosses that boundary.

**Checkpoint:** `npm run bddgen && npx playwright test .features-gen/api` passes
against a real backend — one generated test running all eight steps of the login →
create → update → delete flow in sequence.
...
surface once both layers have at least one passing scenario:

| Command | What it does |
|---|---|
| `npm run bddgen` | regenerates `.features-gen/` from `features/**/*.feature` + `src/bdd/steps/**/*.steps.ts` |
| `npm test` | full suite (`bddgen`, then `playwright test`) |
| `npm run ui` | `.features-gen/ui` only |
| `npm run api` | `.features-gen/api` only |
...
*layer* (which part of the codebase am I testing), using generated-output path
arguments that mirror `features/ui`/`features/api` (Phase 3b) — useful when working on
one layer in isolation. `smoke`/`regression` slice by *criticality*, using `--grep`
against the Gherkin tag conventions from `TestTags` (Phase 2) — useful for a fast
pre-merge check (`smoke`) versus a fuller nightly run (`regression`), completely
independent of which layer a given tagged scenario happens to belong to. A scenario can
be reached by more than one of these scripts simultaneously (e.g. `login.feature`'s
`@smoke`-tagged scenario is included by both `npm run ui` and `npm run smoke`) — that
overlap is intentional, not a design flaw, since the two slicing dimensions (layer vs.
criticality) are orthogonal. Every one of these scripts' `pre*` npm hook (Phase 0) runs
`npm run bddgen` first, so `.features-gen/` is always freshly regenerated from the
current `features/`/`src/bdd/steps/` before Playwright ever runs — there's no
"forgot to regenerate" failure mode.
...
**Why there's no separate `npm run bddgen` step before "Run Playwright tests":** the
`Run Playwright tests` step's `npm run test` already regenerates `.features-gen/`
itself, transparently, via the `pretest` npm hook (Phase 0 / Phase 3b) — adding a
separate `bddgen` step here would just be running the same generation twice. This
workflow needed zero changes to pick up BDD generation; it was already invoking
`npm run test`, and that's the one place the regeneration needed to be wired into.

**Checkpoint:** open a PR — the workflow runs typecheck, then the full suite (which
transparently regenerates `.features-gen/` first via `pretest`), and uploads both
report artifacts regardless of pass/fail.
...
└── .features-gen/      (generated by `npm run bddgen`, gitignored — not part of the repo)
    ├── ui/
    │   ├── authentication/login.feature.spec.js
    │   └── checkout/checkout.feature.spec.js
    └── api/event.feature.spec.js
```
...
```
Phase 0  Scaffold & tooling (npm, tsconfig, ESLint, .gitignore, Playwright browsers, playwright-bdd)
Phase 1  ENV config (envLoader, environments/*.env, playwright.config.ts + defineBddConfig(), global setup/teardown)
...
Phase 3b   ↳ BDD wiring (features/ + src/bdd/steps/ layout, playwright-bdd's test, tag-scoped hooks)
Phase 4  UI layer (locators → components → BasePage/pages → validators → hooks → fixture → feature files + steps)
Phase 5  API layer (types → exceptions → retry/token → engine → services → context/facade → fixture → feature file + steps)
